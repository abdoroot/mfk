...
<div class="card-body p-0">
    <div class="border-bottom pb-3 d-flex justify-content-between align-items-center gap-3 flex-wrap">
        ...
        <div class="w3-third">
                 <a href="{{ route('user-subscriptions.change_order_status_form',['id'=> $orderdata->id ]) }}"
    ...
    <div class="pay-box">
        ...
        <div class="pay-booking-details">
            ...
            <div class="row">
                ...
                <div class="col-sm-6 align-text">
                    <span id="service_schedule__span">{{ $orderdata->date ?? '-'}}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="c1-light-bg radius-10 py-3 px-4 flex-grow-1">
            ...
            <ul class="list-info">
                ...
                <li>
                    ...
                    <p class="customer-info-text">{{ optional($orderdata->customer)->address ?? '-' }}</p>
                </li>
            </ul>
        </div>
                ...
                <tr>
                    <td class="text-wrap ps-lg-3">
                        <div class="d-flex flex-column">
                            <a href=""
                                class="booking-service-link fw-bold">{{optional($orderdata->plan)->title ?? '-'}}</a>
                        </div>
                    </td>
                    <td>{{ isset($orderdata->amount) ? getPriceFormat($orderdata->amount) : 0 }}</td>
                    <td>{{!empty($orderdata->quantity) ? $orderdata->quantity : 0}}</td>
                    <td class="text-end">{{getPriceFormat($orderdata->final_total_service_price)}}</td>
                </tr>
            </tbody>
        </table>
    </div>
                ...
                <table class="table-sm title-color align-right w-100">
                    <tbody>
                        <tr>
                            ...
                            <td class="bk-value">{{getPriceFormat($orderdata->plan->amount)}} * {{$orderdata->quantity}} = {{getPriceFormat($orderdata->final_total_service_price)}}</td>
                        </tr>
                        @if($orderdata->bookingPackage == null)
                        <tr>
                            <td>{{__('messages.discount')}} ({{$orderdata->discount}}% off)</td>
                            <td class="bk-value text-success">-{{getPriceFormat($orderdata->final_discount_amount)}}</td>
                        </tr>
                        @endif
                        @if($orderdata->couponAdded != null)
                        <tr>
                            <td>{{__('messages.coupon')}} ({{($orderdata->couponAdded->code)}})</td>
                            <td class="bk-value text-success">-{{ getPriceFormat($orderdata->final_coupon_discount_amount) }}</td>
                        </tr>
                        @endif
                        <tr class="grand-sub-total">
                            <td>{{__('messages.subtotal_vat')}}</td>
                            <td class="bk-value">{{getPriceFormat($orderdata->final_sub_total)}}</td>
                        </tr>
                    
                        <tr>
                            <td>{{__('messages.tax')}}</td>
                            <td class="text-right text-danger">{{getPriceFormat($orderdata->final_total_tax)}}</td>
                        </tr>
                        ...
                        <tr class="grand-total">
                            ...
                            <td class="bk-value">
                                <h3>{{isset($orderdata->bookingAddonService) ? getPriceFormat($orderdata->total_amount+$addonTotalPrice) : getPriceFormat($orderdata->total_amount)}}</h3>
                            </td>
                        </tr>
                        @if($orderdata->plan->is_enable_advance_payment == 1 )
                        <tr>
                            <td>{{__('messages.advance_payment_amount')}} ({{$orderdata->service->advance_payment_amount}}%)</td>
                            <td class="text-right">{{getPriceFormat($orderdata->advance_paid_amount)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('messages.remaining_amount')}}</td>
                            <td class="text-right">{{getPriceFormat($orderdata->total_amount - $orderdata->advance_paid_amount )}}</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
