...
?>
{{ Form::open(['route' => ['category.destroy', $data->id], 'method' => 'delete','data--submit'=>'category'.$data->id]) }}
<div class="d-flex justify-content-end align-items-center">
    @if(!$data->trashed())
        @if($auth_user->can('category delete'))
        <a class="mr-3 delete-category" href="{{ route('store-category.destroy', $data->id) }}" data--submit="category{{$data->id}}" 
            data--ajax="true"
    ...
    @if(auth()->user()->hasAnyRole(['admin']) && $data->trashed())
        <a href="{{ route('store_category.action',['id' => $data->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('store_category.action',['id' => $data->id, 'type' => 'forcedelete']) }}"
